    ...
    interceptLinks() {
        ...
        links.forEach(link => {
            link.addEventListener('click', (e) => {
                ...
                // 如果選單是開著的，先執行關閉動畫
                if (typeof menuTl !== 'undefined' && document.getElementById('menuOverlay').style.visibility !== 'hidden') {
                    menuTl.reverse();
                }
...
window.addEventListener('load', function () {
    ...
    // Should skipping be allowed? Current requirement: strict wait.
    // But we keep the loop logic for init.
    startLoadingSequence();

    // 更新 UI 的輔助函數
    function updateLoaderUI() {
        if (loaderCounter) loaderCounter.innerText = Math.floor(progressObj.val).toString().padStart(3, '0');
        if (loaderBar) loaderBar.style.width = `${progressObj.val}%`;
    }
